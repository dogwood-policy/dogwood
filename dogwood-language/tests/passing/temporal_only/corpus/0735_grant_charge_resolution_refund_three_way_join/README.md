...
`Grant::resolution` *returned* it), **actually charged** (a past
`Charge::resolution` *completed* for it), and **equals what this refund
...
## Why the charge is a `resolution`, not a `request`

Refunding against a mere *request* would let a charge that never went
through be refunded. Keying clause (2) on `Charge::resolution` means only
...
- **bob** is granted `300` (`@7`) and his charge for `300` is only
  **requested** (`@8`), never resolved:
  - `@9` Refund `300` → **false** — clauses (1) and (3) hold for `300`,
    but no `Charge::resolution` exists, so clause (2) fails. This is the
...
each is `false`. The `resolution` events (`@1`, `@3`, `@7`) are
